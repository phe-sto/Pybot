...
class Pybot:
    ...
    def export_sikuli_class(self, project_name):
        ...
        new_class = "Pybot/{0}".format(file_name)
        ...
        data_to_export = self._export_png(data_to_export)
        file_to_export.close()
        file_to_write = open(new_class, mode="w", encoding="utf-8")
        ...
        for file_name in project_files:
            if file_name.endswith(".png"):
                ...
                copy(img, "img/")
        return path.isfile(new_class)

    def _export_png(self, str_png):
        """
        Change the address of the *.png to img/*.png with substitution of regex

        Args:
            String containing *.png

        Returns:
            img/*.png where was the *.png
        """
        return re.sub(r'([0-9]{13}.png)', r'img/\1', str_png)
...
class PybotTest(unittest.TestCase):
    ...
    def test_F_export_sikuli(self):
        """Test the export of a sikuli project to the Pybot python package"""
        test_automaton = Pybot()
        test_automaton.export_sikuli_class("tahomaBee")
